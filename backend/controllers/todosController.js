...
exports.createDB = (req, res) => {
    let q = 'CREATE DATABASE todolist1';
...
//CREATE TABLE
exports.createTable = (req, res) => {
    let q = 'CREATE TABLE todolist1(id int AUTO_INCREMENT, firstName VARCHAR(255), lastName VARCHAR(255), timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(id))';
    db.query(q, (err, result) => {
        if (err) throw err;
        return res.status(201).json("TABLE CREATED");
    });
...
exports.updateTodo = (req, res) => {
    const { firstName, lastName } = req.body;
    const timestamp = new Date().toISOString().slice(0, 19).replace('T', ' '); // Current timestamp in MySQL format

    const q = `UPDATE todolist1 SET firstName = ?, lastName = ?, timestamp = ? WHERE id = ?`;

    db.query(q, [firstName, lastName, timestamp, req.params.id], (err, result) => {
        if (err) return res.json(err);
        return res.status(200).json(result);
    });
}
